...
def DateTimeFrom(*args,**kws):
    ...
    if len(args) == 1:
        ...
        if len(args) == 2 and \
           type(args[0]) is types.StringType and \
           type(args[1]) is types.StringType:
            # interpret as date and time string
            import Parser
            return Parser.DateTimeFromString(args[0] + ' ' + args[1])
...
def DateTimeDeltaFrom(*args,**kws):
    ...
    if len(args) == 1:
        ...
        if type(arg) is types.StringType:
            import Parser
            return Parser.DateTimeDeltaFromString(arg)
        elif type(arg) is DateTimeDeltaType:
            return arg
        elif type(arg) is DateTimeType:
        ...
        return apply(DateTimeDelta,args)
...
def TimeDeltaFrom(*args,**kws):
    ...
    if len(args) > 1:
        ...
        # days part !
        return apply(DateTimeDelta,(0,)+args)
    else:
...
class RelativeDateTime:
    ...
    def __radd__(self,other,
                 # Locals:
                 waste=None,isinstance=isinstance,DateTimeType=DateTimeType,
    ...
    def __rsub__(self,other,
                 # Locals:
                 waste=None,isinstance=isinstance,DateTime=DateTime):
...
def RelativeDateTimeFrom(*args,**kws):
    ...
    if len(args) == 1:
        ...
        if type(arg) is types.StringType:
            import Parser
            return Parser.RelativeDateTimeFromString(arg)
