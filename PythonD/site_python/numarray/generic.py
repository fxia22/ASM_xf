...
$Id: generic.py,v 1.80 2005/10/21 18:01:50 teoliphant Exp $
"""
...
class NDArray(_ndarray):
    ...
    def __get_array_shape__(self):
        return self._shape

    __array_shape__ = property(__get_array_shape__, None, "")

    def __get_array_data__(self):
	mem = repr(self._data)
	ind = mem.find('at 0x')
	lind = mem.find(' ',ind+4)
        base = mem[ind+3:lind]
        if self._byteoffset == 0:
            ...
            ret=hex(long(base,base=16)+self._byteoffset)[:-1].lower()
	readonly = not (self._flags & _WRITABLE)
	return (ret, readonly)

    __array_data__ = property(__get_array_data__, None, "")

    def __get_array_strides__(self):
        return self._strides

    __array_strides__ = property(__get_array_strides__, None, "")
